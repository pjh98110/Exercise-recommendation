...
selected_survey = st.selectbox(
    "원하는 추천 방식을 선택하세요.",
    options=["추천시스템 기반 맞춤형 운동 추천", "바드 API를 활용한 맞춤형 운동 추천", "최적화 모델 기반 맞춤형 운동 추천"],
...
if selected_survey == "최적화 모델 기반 맞춤형 운동 추천":
    ...
    question71 = st.multiselect("[최적화 모델] 관심있는 신체 지수를 선택하세요.", 
                                ["신장", "체중", "체지방률", "허리둘레", "이완기최저혈압", "수축기최고혈압", "BMI", "허벅지_좌", "허벅지_우"], 
                                placeholder="중복 선택이 가능합니다.", key="b1")

    # 선택된 각 항목에 대해 가중치 입력 요청
    for item in question71:
        st.number_input(f"[최적화 모델] {item}에 대한 원하는 가중치를 입력하세요.(증가는 플러스, 감소는 마이너스를 입력하세요.)", placeholder="__", key=f'b_{item}')
    ...
    if st.button("제출"):
        ...
        st.markdown(f"당신의 성별은 {selected_gender}이며, 연령대는 {selected_age}입니다.")
        st.markdown(f"**SLSQP 알고리즘으로 최적화 결과 추천 운동은 :green[{model_exercises}] 입니다.**")
        st.markdown(f"추천 운동을 기반으로 원하는 정보를 선택하세요")
